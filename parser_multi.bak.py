...
'''
This function will return text from a file. Currently supports .txt, .md, .json, .xml, 
.docx, .pptx, and .pdf files.
...
'''
This function will return text from a pdf file. It does not read any images. And it does not 
...
'''
This function will return text from a docx file. It does not read any images or headers/footers.
...
def parse_docx(file, filename):
    ...
    # The docx package breaks the document into different parts. Here we iterate over the paragraphs
...
def parse_txt(file, filename):
    ...
    try:
        ...
        # Fallback to reading as ISO-8859-1 encoding if UTF-8 fails
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("Usage: python3 parser_multi.py <file_path> <file_name>")
        sys.exit(1)
